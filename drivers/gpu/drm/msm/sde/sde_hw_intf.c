	...
	const struct intf_avr_params *avr_params)
{
	...
	pr_debug("[%s] the qsync min fps is %d, current fps is %d",__func__,
	                         avr_params->min_fps, avr_params->default_fps);
	...
	pr_debug("[%s] add_porches is %d, vsync_period_slow  is %d,avr_vtotal is %d",__func__,
	                                    add_porches, vsync_period_slow, avr_vtotal);
